import type {
  ...
  Selection,
  FixedSlotRuleConfig,
  FixedDurationRuleConfig,
  MinMaxDurationRuleConfig
} from './types';
...
export class BaseRule implements BookingRule {
  ...
  applies(dayIdx: number, studioIdx: number, startHour: number, endHour: number): boolean {
    ...
    return (
      (allDays || this.days.includes(dayIdx)) &&
      (allStudios ||
        this.studios.includes(studioIdx) ||
  ...
  calculateSelection(
    ...
    timeIdx: number,
    studioIdx: number,
    isBooked: (dayIdx: number, timeIdx: number, studioIdx: number) => boolean
  ): Selection {
...
export class FixedSlotRule extends BaseRule {
  ...
  validate(dayIdx: number, studioIdx: number, startHour: number, endHour: number): boolean {
    ...
    return this.slots.some(
      ([slotStart, slotEnd]) => startHour === slotStart && endHour === slotEnd - 1
    );
  }
  ...
  calculateSelection(
    ...
    const slot = this.slots.find(([start, end]) => timeIdx >= start && timeIdx < end);
  ...
  extendSelection(
    ...
    const isMultipleOfSlotSize =
      this.slots.length > 0 &&
      this.slots[0][1] - this.slots[0][0] > 0 &&
...
export class FixedDurationRule extends BaseRule {
  ...
  calculateSelection(
    ...
    const maxPossibleEnd = Math.min(start + this.duration - 1, this.endHour - 1);
  ...
  extendSelection(
    ...
    if (
      !this.applies(
        dayIdx,
        studioIdx,
        Math.min(startHourIdx, newTimeIdx),
        Math.max(endHourIdx, newTimeIdx)
      )
    ) {
      return existingSelection; // Can't extend - rules don't apply
    }
...
export class BookingRuleEngine {
  ...
  getApplicableRules(dayIdx: number, studioIdx: number, timeIdx?: number): BookingRule[] {
    ...
    const applicableRules = this.rules.filter((rule) =>
      rule.applies(
        dayIdx,
        studioIdx,
        timeIdx !== undefined ? timeIdx : 0,
        timeIdx !== undefined ? timeIdx : 23
      )
    );
  ...
  validateSelection(selection: Selection): boolean {
    ...
    const applicableRules = this.getApplicableRules(selection.dayIdx, selection.studioIdx);
  ...
  getStudioRuleDescriptions(studioIdx: number): string[] {
    ...
    const studioRules = this.rules.filter((rule) => {
      ...
      const appliesTo =
        rule.studios.length === 0 || // Empty means all studios
